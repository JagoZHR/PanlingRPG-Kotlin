...
import com.panling.basic.skill.strategy.wood.*
import com.panling.basic.util.MageUtil
import org.bukkit.entity.Player
import org.bukkit.event.entity.EntityDamageByEntityEvent

class MageWoodSkill(private val plugin: PanlingBasic) : AbstractSkill("MAGE_WOOD", "千丝", PlayerClass.MAGE) {

    private val strategies: MutableMap<String, MageSkillStrategy> = HashMap()

    init {
        registerStrategy("OFFENSE_UNCOMMON_FURNACE", WoodAttackT3Strategy(plugin))
        registerStrategy("SUPPORT_UNCOMMON_FURNACE", WoodSupportT3Strategy(plugin))
        registerStrategy("OFFENSE_RARE_FURNACE", WoodAttackT4FurnaceStrategy(plugin))
        registerStrategy("SUPPORT_RARE_FURNACE", WoodSupportT4FurnaceStrategy(plugin))
        registerStrategy("OFFENSE_EPIC_FURNACE", WoodAttackT5FurnaceStrategy(plugin))
        registerStrategy("SUPPORT_EPIC_FURNACE", WoodSupportT5FurnaceStrategy(plugin))
    }
